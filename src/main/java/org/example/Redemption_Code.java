...
//        return screenshotPath;
    ...
    @Test(priority = 4)
    public void Redemption_code() throws IOException {
        ExtentTest test = extent.createTest("Redemption_code");
        long startTime = System.currentTimeMillis();
        try {
            Thread.sleep(3000);
            BenepikPlusWebPage page = new BenepikPlusWebPage(driver);
            page.Redemption();
            test.log(Status.PASS, "Redemption code Passed");
        } catch (Exception e) {
            test.log(Status.FAIL, "Redemption code Failed: " + e.getMessage());
            Assert.fail("Redemption code Failed: " + e.getMessage());
        } finally {
            long duration = System.currentTimeMillis() - startTime;
            test.log(Status.INFO, "Test duration: " + duration + " milliseconds");
            String screenshotPath = captureScreenshot("RedemptionCode");
            test.addScreenCaptureFromPath(screenshotPath);
            extent.flush();
        }
    }
